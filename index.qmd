...
#| fig-width: 10 # This controls RMarkdown display size
#| fig-height: 8  # This controls RMarkdown display size
...
# Check if figure_output_dir exists (it should if setup chunk ran)
if (!exists("figure_output_dir") || !dir.exists(figure_output_dir)) {
  cat("Warning: 'figure_output_dir' not defined or does not exist. Plots will not be saved to file.\n")
  # Optionally, define it here as a fallback, though it's better in setup
  # figure_output_dir <- file.path(config$base_dir, "figure_files")
  # if (!dir.exists(figure_output_dir)) dir.create(figure_output_dir, recursive = TRUE)
}

# Define plot parameters (can be customized)
plot_params <- list(
  col = hcl.colors(255, "viridis"), # Using viridis palette
  plg = list(loc = "right", title = "Richness", cex = 0.9, title.cex=0.9),
  pax = list(cex.axis = 0.9, cex.lab=0.9)
)
# Define common plot dimensions for saved files
png_width <- 1000 # pixels
png_height <- 750 # pixels
png_res <- 100    # dpi


if (!is.null(host_richness_sum_cropped)) {
  # Plot to RMarkdown output first
  ...
  # Save the plot if directory exists
  if (exists("figure_output_dir") && dir.exists(figure_output_dir)) {
    host_plot_filename <- file.path(figure_output_dir, "host_richness_current_cropped.png")
    png(filename = host_plot_filename, width = png_width, height = png_height, units = "px", res = png_res)
    plot(host_richness_sum_cropped, 
         main = "Summed Host Anemone Richness (Current, Cropped)", 
         col = plot_params$col, 
         plg = plot_params$plg, 
...
if (!is.null(fish_richness_sum_cropped)) {
  ...
  if (exists("figure_output_dir") && dir.exists(figure_output_dir)) {
    ...
    png(filename = fish_plot_filename, width = png_width, height = png_height, units = "px", res = png_res)
    plot(fish_richness_sum_cropped, 
         main = "Summed Anemonefish Richness (Current, Env-Only, Cropped)",
         col = plot_params$col, 
         plg = plot_params$plg, 
...
# Classify based on host count: <=3 hosts = Specialist, >3 hosts = Generalist
anemonefish_specialization_df <- anemonefish_host_counts %>%
  mutate(specialization_type = factor(ifelse(n_hosts <= 3, "Specialist", "Generalist"), # Updated condition
...
if (nrow(auc_improvement_df) > 0 && 
  ...
  plot_auc_improvement <- ggplot(auc_improvement_df, aes(x = specialization_type, y = AUC_improvement, fill = specialization_type)) +
    ...
    labs(
      title = "AUC Improvement by Host Specialisation",
      subtitle = "Comparing PCA Only vs. PCA + Host",
      x = "Anemonefish Specialisation Type", # Updated X-axis label
      y = expression(Delta * " AUC (PCA + Host)")
    ) +
...
if (!is.null(df_fish_comparison) && nrow(df_fish_comparison) > 0 && "AUC_test_CV" %in% names(df_fish_comparison)) {
  ...
  # Model: AUC_test_CV ~ model_type + (1|species)
  ...
  plot_auc_comparison <- ggplot(df_fish_comparison, aes(x = model_type, y = AUC_test_CV, fill = model_type)) +
  ...
  plot_auc_per_species <- ggplot(df_fish_comparison_plot, aes(x = species_display, y = AUC_test_CV, fill = model_type)) +
  ...
  anemonefish_auc_summary_table <- df_fish_comparison %>%
    ...
    dplyr::summarize(
      ...
      n_CV_folds = n(), # This shows how many CV fold results were averaged
      .groups = 'drop'
    ) %>%
    # Clean up species names for display if they are still Genus_species
  ...
  anemonefish_cv_summary_path <- file.path(config$base_dir, "outputs_for_analysis", "anemonefish_detailed_CV_summary_auc.csv")
  dir.create(dirname(anemonefish_cv_summary_path), recursive = TRUE, showWarnings = FALSE)
...
# plot(host_richness_future_list[["current"]], main = "Host Richness - Current")
plot(host_richness_future_list[["ssp119_2050"]], main = "Host Richness - SSP1-1.9 (2050)")
...
# plot(fish_richness_list[["current"]], main = "Anemonefish Richness - Current")
plot(fish_richness_future_list[["ssp119_2050"]], main = "Anemonefish Richness - SSP1-1.9 (2050)")
...
#| label: future-host-fish-overlap-specialization-final
#| message: false
#| warning: false
#| fig-width: 11 # Adjusted for potentially wider facets
#| fig-height: 7

cat("\n--- Calculating Future Host-Anemonefish Overlap Changes by Specialization (Final Version) ---\n")

# Ensure raster package is loaded
if (!requireNamespace("raster", quietly = TRUE)) {
  install.packages("raster")
}
library(raster) # For raster::stack()

# --- Prerequisite Checks ---
required_objects <- c("host_pred_stack_cropped", "fish_pred_stack_cropped",
                      "host_pred_stacks_future_individual", "fish_pred_stacks_future_individual",
                      "anemonefish_specialization_df", "host_associations_df",
                      "future_scenarios_to_load", "config", "scenario_label_converter")
for (obj_name in required_objects) {
  if (!exists(obj_name) || is.null(get(obj_name))) {
    stop(paste("Prerequisite object '", obj_name, "' not found or is NULL. Ensure previous chunks have run successfully.", sep=""))
  }
}
if(!is.list(host_pred_stacks_future_individual) || !is.list(fish_pred_stacks_future_individual)){
    stop("Future individual prediction stacks must be lists.")
}

# --- Initialize results dataframe ---
overlap_change_results_list <- list()

current_anemonefish_names <- names(fish_pred_stack_cropped)
current_host_names <- names(host_pred_stack_cropped)

# --- Loop through each anemonefish species ---
for (fish_species_name in current_anemonefish_names) {

  cat("\nProcessing Anemonefish:", fish_species_name, "\n")

  fish_specialization_info <- anemonefish_specialization_df %>% filter(species == fish_species_name)
  if (nrow(fish_specialization_info) == 0) {
    cat("  Warning: No specialization info found for", fish_species_name, ". Skipping.\n")
    next
  }
  specialization_type <- fish_specialization_info$specialization_type[1]

  current_fish_raster_orig <- fish_pred_stack_cropped[[fish_species_name]]
  if (is.null(current_fish_raster_orig)) {
    cat("  Warning: Current prediction raster not found for", fish_species_name, ". Skipping.\n")
    next
  }

  documented_hosts <- host_associations_df %>%
    filter(AnemonefishScientificName == fish_species_name) %>%
    pull(AssociatedAnemoneScientificName) %>%
    unique()
  documented_hosts <- intersect(documented_hosts, current_host_names)

  if (length(documented_hosts) == 0) {
    cat("  Warning: No documented hosts found (or hosts not in current_host_names) for", fish_species_name, ". Skipping.\n")
    next
  }

  current_overlaps_d_values <- c()

  if (specialization_type == "Specialist") {
    for (host_name in documented_hosts) {
      current_host_raster_single_orig <- host_pred_stack_cropped[[host_name]]
      if (!is.null(current_host_raster_single_orig)) {
        current_fish_raster_comp <- current_fish_raster_orig
        current_host_raster_single_comp <- current_host_raster_single_orig
        if(!terra::compareGeom(current_fish_raster_comp, current_host_raster_single_comp, stopOnError=FALSE, res=TRUE,crs=TRUE,ext=TRUE)){
            current_host_raster_single_comp <- terra::resample(current_host_raster_single_comp, current_fish_raster_comp, method="bilinear")
        }
        overlap_stack_curr_terra <- c(current_fish_raster_comp, current_host_raster_single_comp)
        names(overlap_stack_curr_terra) <- c(fish_species_name, host_name)
        overlap_stack_curr_raster <- raster::stack(overlap_stack_curr_terra)
        current_overlaps_d_values <- c(current_overlaps_d_values, ENMeval::calc.niche.overlap(overlap_stack_curr_raster, "D")[1,2])
      }
    }
    mean_current_overlap_d <- ifelse(length(current_overlaps_d_values) > 0, mean(current_overlaps_d_values, na.rm = TRUE), NA_real_)
  } else { # Generalist
    current_generalist_host_rasters_list <- list()
    for(hn in documented_hosts){
        if(!is.null(host_pred_stack_cropped[[hn]])){
            current_generalist_host_rasters_list[[hn]] <- host_pred_stack_cropped[[hn]]
        }
    }
    if (length(current_generalist_host_rasters_list) > 0) {
        temp_host_stack <- terra::rast(current_generalist_host_rasters_list)
        processed_host_stack_for_composite <- temp_host_stack # Initialize
        if(terra::nlyr(temp_host_stack) > 0) { # Ensure there are layers to process
            resampled_layers_for_composite <- list()
            for(k in 1:terra::nlyr(temp_host_stack)){
                layer_k <- temp_host_stack[[k]]
                if(!terra::compareGeom(current_fish_raster_orig, layer_k, stopOnError=FALSE, res=TRUE,crs=TRUE,ext=TRUE)){
                    layer_k <- terra::resample(layer_k, current_fish_raster_orig, method="bilinear")
                }
                resampled_layers_for_composite[[k]] <- layer_k
            }
            processed_host_stack_for_composite <- rast(resampled_layers_for_composite)
        }
      composite_current_host_raster <- max(processed_host_stack_for_composite, na.rm = TRUE)
      names(composite_current_host_raster) <- "CompositeHost"
      current_fish_raster_comp <- current_fish_raster_orig
      overlap_stack_curr_terra <- c(current_fish_raster_comp, composite_current_host_raster)
      names(overlap_stack_curr_terra) <- c(fish_species_name, "CompositeHost")
      overlap_stack_curr_raster <- raster::stack(overlap_stack_curr_terra)
      mean_current_overlap_d <- ENMeval::calc.niche.overlap(overlap_stack_curr_raster, "D")[1,2]
    } else {
      mean_current_overlap_d <- NA_real_
    }
  }

  for (scenario_name in future_scenarios_to_load) {
    cat("    Scenario:", scenario_name, "\n")
    future_fish_raster_orig <- fish_pred_stacks_future_individual[[scenario_name]][[fish_species_name]]
    future_host_rasters_all_orig <- host_pred_stacks_future_individual[[scenario_name]]

    if (is.null(future_fish_raster_orig) || is.null(future_host_rasters_all_orig)) {
      cat("      Warning: Future fish or host stack missing for scenario", scenario_name, "and fish", fish_species_name, ". Skipping.\n")
      next
    }

    future_overlaps_d_values <- c()
    if (specialization_type == "Specialist") {
      for (host_name in documented_hosts) {
        future_host_raster_single_orig <- future_host_rasters_all_orig[[host_name]]
        if (!is.null(future_host_raster_single_orig)) {
            future_fish_raster_comp <- future_fish_raster_orig
            future_host_raster_single_comp <- future_host_raster_single_orig
            if(!terra::compareGeom(future_fish_raster_comp, future_host_raster_single_comp, stopOnError=FALSE, res=TRUE,crs=TRUE,ext=TRUE)){
                future_host_raster_single_comp <- terra::resample(future_host_raster_single_comp, future_fish_raster_comp, method="bilinear")
            }
          overlap_stack_fut_terra <- c(future_fish_raster_comp, future_host_raster_single_comp)
          names(overlap_stack_fut_terra) <- c(fish_species_name, host_name)
          overlap_stack_fut_raster <- raster::stack(overlap_stack_fut_terra)
          future_overlaps_d_values <- c(future_overlaps_d_values, ENMeval::calc.niche.overlap(overlap_stack_fut_raster, "D")[1,2])
        }
      }
      mean_future_overlap_d <- ifelse(length(future_overlaps_d_values) > 0, mean(future_overlaps_d_values, na.rm = TRUE), NA_real_)
    } else { # Generalist
      future_generalist_host_rasters_list <- list()
        for(hn in documented_hosts){
            if(!is.null(future_host_rasters_all_orig[[hn]])){
                future_generalist_host_rasters_list[[hn]] <- future_host_rasters_all_orig[[hn]]
            }
        }
      if (length(future_generalist_host_rasters_list) > 0) {
          temp_fut_host_stack <- rast(future_generalist_host_rasters_list)
          processed_fut_host_stack_for_composite <- temp_fut_host_stack # Initialize
          if(terra::nlyr(temp_fut_host_stack) > 0 ) { # Ensure there are layers
              resampled_fut_layers_for_composite <- list()
              for(k in 1:terra::nlyr(temp_fut_host_stack)){
                  layer_k_fut <- temp_fut_host_stack[[k]]
                  if(!terra::compareGeom(future_fish_raster_orig, layer_k_fut, stopOnError=FALSE, res=TRUE,crs=TRUE,ext=TRUE)){
                      layer_k_fut <- terra::resample(layer_k_fut, future_fish_raster_orig, method="bilinear")
                  }
                  resampled_fut_layers_for_composite[[k]] <- layer_k_fut
              }
              processed_fut_host_stack_for_composite <- rast(resampled_fut_layers_for_composite)
          }
        composite_future_host_raster <- max(processed_fut_host_stack_for_composite, na.rm = TRUE)
        names(composite_future_host_raster) <- "CompositeHost"
        future_fish_raster_comp <- future_fish_raster_orig
        overlap_stack_fut_terra <- c(future_fish_raster_comp, composite_future_host_raster)
        names(overlap_stack_fut_terra) <- c(fish_species_name, "CompositeHost")
        overlap_stack_fut_raster <- raster::stack(overlap_stack_fut_terra)
        mean_future_overlap_d <- ENMeval::calc.niche.overlap(overlap_stack_fut_raster, "D")[1,2]
      } else {
        mean_future_overlap_d <- NA_real_
      }
    }

    overlap_change <- ifelse(!is.na(mean_future_overlap_d) && !is.na(mean_current_overlap_d),
                             mean_future_overlap_d - mean_current_overlap_d,
                             NA_real_)

    overlap_change_results_list[[length(overlap_change_results_list) + 1]] <- data.frame(
      anemonefish_species = fish_species_name,
      specialization_type = specialization_type,
      n_documented_hosts = length(documented_hosts),
      scenario = scenario_name,
      current_overlap_D = mean_current_overlap_d,
      future_overlap_D = mean_future_overlap_d,
      change_in_overlap_D = overlap_change
    )
  }
}

all_overlap_changes_df <- dplyr::bind_rows(overlap_change_results_list)
...
if(nrow(all_overlap_changes_df) > 0) {
  cat("Summary of change_in_overlap_D before filtering:\n")
  print(summary(all_overlap_changes_df$change_in_overlap_D))
  cat("Table of scenarios before filtering change_in_overlap_D:\n")
  print(table(all_overlap_changes_df$scenario, useNA="ifany"))
  cat("Table of specialization_type before filtering change_in_overlap_D:\n")
  print(table(all_overlap_changes_df$specialization_type, useNA="ifany"))
}

data_for_plot <- all_overlap_changes_df %>% filter(!is.na(change_in_overlap_D))
cat("\nNumber of rows in data_for_plot (after filtering NA change_in_overlap_D):", nrow(data_for_plot), "\n")

if(nrow(data_for_plot) > 0) {
  cat("Table of scenarios in data_for_plot:\n")
  print(table(data_for_plot$scenario, useNA="ifany"))
  cat("Table of specialization_type in data_for_plot:\n")
  print(table(data_for_plot$specialization_type, useNA="ifany"))
  cat("First few rows of data_for_plot:\n")
  print(head(data_for_plot))

  summary_overlap_change <- data_for_plot %>% # Use filtered data for summary too
    group_by(scenario, specialization_type) %>%
    summarise(
      mean_change_D = mean(change_in_overlap_D, na.rm = TRUE), # na.rm = TRUE still good practice
      sd_change_D = sd(change_in_overlap_D, na.rm = TRUE),
      n_species_in_group = n_distinct(anemonefish_species),
      .groups = "drop"
    ) %>% arrange(scenario, specialization_type)
  cat("\nSummary of Mean Change in Schoener's D Overlap by Specialization and Scenario (from filtered data):\n")
  print(summary_overlap_change)

  plot_overlap_change <- ggplot(data_for_plot,
                                aes(x = specialization_type, y = change_in_overlap_D, fill = specialization_type)) +
    geom_boxplot(alpha = 0.8, outlier.shape = NA, width=0.7) +
    geom_jitter(width = 0.15, alpha = 0.5, size = 2) +
    stat_summary(fun = mean, geom = "point", shape = 18, size = 3.5, color = "black",
                 position = position_dodge(width = 0.75)) +
    facet_wrap(~ scenario, labeller = labeller(scenario = scenario_label_converter), scales="free_y") +
    geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
    labs(
      title = "Change in Host-Anemonefish Niche Overlap (Schoener's D)",
      subtitle = "Future vs. Current, by Anemonefish Specialization",
      x = "Anemonefish Specialization Type (≤3 hosts = Specialist; >3 hosts = Generalist)",
      y = expression(Delta * " Schoener's D (Future - Current)")
    ) +
    scale_fill_manual(values = c("Generalist" = "cornflowerblue", "Specialist" = "darkorange"),
                      name = "Specialization") +
    theme_bw(base_size = 11) +
  ...
  print(plot_overlap_change)

  # Statistical test
  example_scenario_for_test <- future_scenarios_to_load[1]
  test_data_overlap_change <- data_for_plot %>%
    filter(scenario == example_scenario_for_test)

  if (nrow(test_data_overlap_change) > 2 &&
      !all(is.na(test_data_overlap_change$specialization_type)) &&
      length(unique(na.omit(test_data_overlap_change$specialization_type))) == 2 &&
      sum(test_data_overlap_change$specialization_type == "Generalist", na.rm=TRUE) > 1 &&
      sum(test_data_overlap_change$specialization_type == "Specialist", na.rm=TRUE) > 1 ) {

    cat(paste0("\n--- T-test for Change in Overlap D (", scenario_label_converter[example_scenario_for_test], ") between Generalists and Specialists ---\n"))
    ttest_overlap_change <- t.test(change_in_overlap_D ~ specialization_type, data = test_data_overlap_change)
    print(ttest_overlap_change)
  } else {
    cat(paste0("\nNot enough data for t-test on overlap change for scenario: ", example_scenario_for_test, "\n"))
    if(nrow(test_data_overlap_change) > 0){
        cat("  N Generalist:", sum(test_data_overlap_change$specialization_type == "Generalist", na.rm=TRUE),
            " N Specialist:", sum(test_data_overlap_change$specialization_type == "Specialist", na.rm=TRUE), "\n")
    } else {
        cat("  No data for this scenario after filtering NAs in change_in_overlap_D.\n")
    }
  }
} else {
  cat("No overlap change results to display or summarize after filtering NAs.\n")
}
```
...
if (is.null(host_richness_sum_cropped) || is.null(fish_richness_sum_cropped)) {
  ...
  if (exists("df_host_change_long") && exists("df_fish_change_long") && 
    ...
    df_host_change_long$guild <- "Host Anemone"
    df_fish_change_long$guild <- "Anemonefish"
...
#| message: false
#| warning: false
#| fig-width: 12 # Might need to adjust depending on final plot layout
#| fig-height: 8  # Might need to adjust
...
# Ensure anemonefish_specialization_df from Chunk A is available
...
host_breadth_shifts_df_final <- NULL # Initialize
if (nrow(host_niche_breadth_df) > 0) {
  ...
  # Perform and print correlations for hosts
  for (scenario_name in future_scenarios_to_load) {
    shift_col_name <- paste0("shift_", gsub("ssp", "", scenario_name))
...
fish_breadth_shifts_df_final <- NULL # Initialize
if (nrow(fish_niche_breadth_df) > 0) {
  fish_breadth_shifts_df_temp <- fish_niche_breadth_df %>%
    dplyr::select(species_name_sanitized, breadth_B2 = levins_B2) %>%
    # Merge with specialization type (species names should match after underscore conversion in previous chunk)
  ...
  fish_breadth_shifts_df_final <- fish_breadth_shifts_df_temp %>% 
    filter(!is.na(breadth_B2), !is.na(specialization_type)) # Also filter out if specialization type is NA
    
  cat("Anemonefish (env-only) data prepared for breadth vs. shift analysis (with specialization):\n")
  print(head(fish_breadth_shifts_df_final))

  # Perform and print correlations for fish, separately for Generalists and Specialists
  for (scenario_name in future_scenarios_to_load) {
    shift_col_name <- paste0("shift_", gsub("ssp", "", scenario_name))
...
if (!is.null(host_breadth_shifts_df_final) && nrow(host_breadth_shifts_df_final) > 0) {
  host_plot_df_final <- host_breadth_shifts_df_final %>% 
    ...
    mutate(guild = "Host Anemone",
           specialization_type = "Host") # Add a dummy specialization for faceting
...
if (!is.null(fish_breadth_shifts_df_final) && nrow(fish_breadth_shifts_df_final) > 0) {
  fish_plot_df_final <- fish_breadth_shifts_df_final %>%
    pivot_longer(cols = starts_with("shift_"), 
                 names_to = "scenario_shift_code", 
                 values_to = "suitability_shift") %>%
    mutate(guild = "Anemonefish") # Specialization type is already there
...
if (!is.null(all_breadth_shifts_long_df_final) && nrow(all_breadth_shifts_long_df_final) > 0) {
  all_breadth_shifts_long_df_final <- all_breadth_shifts_long_df_final %>%
    ...
    mutate(facet_group = paste(guild, specialization_type, sep=" - ")) # For faceting
  ...
  plot_breadth_vs_shift_specialization <- ggplot(all_breadth_shifts_long_df_final, 
    ...
    geom_point(alpha = 0.7, aes(color = specialization_type)) + # Color points by specialization_type
    geom_smooth(method = "lm", se = TRUE, aes(linetype = specialization_type, color=specialization_type)) + # Separate lines
...
# Note: The individual plots for Host and Fish from 'Figure: Niche Breadth vs. Suitability Shift' 
# can still be generated as before if desired, or this combined plot can replace them.
# The key new part is the separate correlation tests for generalist/specialist anemonefish.
```
...
# --- Host Anemones Plot ---
# Use the _final dataframe name here
if (!is.null(host_breadth_shifts_df_final) && chosen_scenario_for_figure %in% names(host_breadth_shifts_df_final) && "breadth_B2" %in% names(host_breadth_shifts_df_final)) {
  ...
  plot_a_host <- ggplot(host_breadth_shifts_df_final, aes(x = .data[[chosen_scenario_for_figure]], y = breadth_B2)) + # Use _final
    geom_point(alpha = 0.8, color = "dodgerblue3") +
    geom_smooth(method = "lm", 
                linetype = if (!is.null(host_cor_test) && host_cor_test$p.value < 0.05) "solid" else "dashed", 
                se = TRUE, color = "black") + 
    labs(
      # title = "Host Anemones", # Title can be part of the combined plot title
  ...
  if(!is.null(host_cor_test)) print(host_cor_test) # This will print the correlation test from the PREVIOUS chunk
...
# Use the _final dataframe name here
# Also, this plot in your original setup was for ALL anemonefish (env-only),
# not split by specialization. If you want to maintain that, use fish_breadth_shifts_df_final
# but filter it for env-only model types if that distinction was ever made in that df.
# However, fish_breadth_shifts_df_final in your updated chunk already contains specialization_type.
# For a plot analogous to the ant paper's Figure 5 (which didn't split by specialization for this specific plot type),
# you'd plot all anemonefish together here.
# If you wanted to show generalist vs specialist here side-by-side FOR THIS SCENARIO, that would be a different plot.
# Let's assume this is for all anemonefish combined for direct comparison with hosts.
...
if (!is.null(fish_breadth_shifts_df_final) && chosen_scenario_for_figure %in% names(fish_breadth_shifts_df_final) && "breadth_B2" %in% names(fish_breadth_shifts_df_final)) {
  ...
  # If you want to show ALL fish regardless of specialization for this particular plot panel:
  cor_data_fish_all <- fish_breadth_shifts_df_final %>% # Use the df that has all fish
  ...
  plot_b_fish <- ggplot(fish_breadth_shifts_df_final, aes(x = .data[[chosen_scenario_for_figure]], y = breadth_B2)) + # Use _final
  ...
  if(!is.null(fish_cor_test_all)) print(fish_cor_test_all) # Prints the correlation for ALL fish
...
if (!is.null(plot_a_host) && !is.null(plot_b_fish)) {
  ...
  combined_title <- paste("Figure: Niche Breadth vs. Suitability Shift for", gsub("shift_", "SSP", chosen_scenario_for_figure))
  
  combined_breadth_plot <- cowplot::plot_grid(plot_a_host + labs(title="Host Anemones"), 
                                              plot_b_fish + labs(title="Anemonefish"), 
                                              ncol = 2, labels = "AUTO")
  
  # Add a super title to the combined plot
  final_plot_with_title <- cowplot::ggdraw() +
    cowplot::draw_plot(combined_breadth_plot) +
    cowplot::draw_label(combined_title, fontface = 'bold', x = 0.5, y = 0.97, hjust = 0.5)
...
if (length(host_vi_files) == 0 && env_model_predictor_suffix == "_pca") { # Specific check for typical pca subfolder in target_vi_base
    host_vi_dir_alt <- file.path(host_vi_dir, "anemone_pca")
...
if (length(fish_vi_files) == 0 && env_model_predictor_suffix == "_pca") { # Specific check
    fish_vi_dir_alt <- file.path(fish_vi_dir, "anemonefish_pca")
...
if (!is.null(host_vi_data) && !is.null(fish_vi_data) && nrow(host_vi_data) > 0 && nrow(fish_vi_data) > 0) {
  ...
  plot_vi_comparison <- ggplot(combined_vi_data, aes(x = Variable, y = Permutation_Importance, fill = guild)) +
    ...
    scale_fill_manual(values = c("Host Anemone" = "coral", "Anemonefish" = "steelblue"), name = "Guild") +
    theme_bw(base_size = 12) +
  ...
  for (var_name in predictor_vars_to_test) {
    ...
    if (nrow(data_for_ttest %>% distinct(guild)) == 2 && 
        nrow(data_for_ttest %>% filter(guild == "Host Anemone") %>% na.omit()) > 1 &&
        nrow(data_for_ttest %>% filter(guild == "Anemonefish") %>% na.omit()) > 1) {
